...
// THE SIBLING SEARCH MUST NOT DEPEND ON HOW DEEP THE TOOL SITS. The first version counted three
// directories up, because that is where `plugins/vc-kb` lives inside a consumer repository, and so
// it resolved from exactly one layout. It missed from the lab the tool was built in, and it would
// have missed from any pair of clones sitting side by side. These two cases are the two layouts
// that actually exist.
test('the sibling base is found from the lab layout and from the plugin layout alike', () => {
  const root = mkdtempSync(join(tmpdir(), 'kb-layout-'));
  const corpus = join(root, 'vc-knowledge');
  mkdirSync(corpus, { recursive: true });
  writeFileSync(join(corpus, 'kb.json'), '{}');

  const lab = join(root, 'vc-kb-lab');
  const plugin = join(root, 'vc-mcp-testing-module', 'plugins', 'vc-kb');
  mkdirSync(lab, { recursive: true });
  mkdirSync(plugin, { recursive: true });

  assert.equal(resolveBase({ here: lab, env: {} }), corpus, 'lab layout');
  assert.equal(resolveBase({ here: plugin, env: {} }), corpus, 'plugin layout');

  rmSync(root, { recursive: true, force: true });
});

test('the climb stops rather than wandering to the filesystem root', () => {
  const root = mkdtempSync(join(tmpdir(), 'kb-noclimb-'));
  const deep = join(root, 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h');
  mkdirSync(deep, { recursive: true });
  assert.equal(resolveBase({ here: deep, env: {} }), null, 'no base anywhere above it');
  rmSync(root, { recursive: true, force: true });
});
